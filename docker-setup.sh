...
docker network create msa-network 2>/dev/null || echo "네트워크가 이미 존재합니다."

# 기존 컨테이너 정리
echo "2. 기존 컨테이너 정리 중..."
docker stop userservice postservice 2>/dev/null || true
docker rm userservice postservice 2>/dev/null || true
